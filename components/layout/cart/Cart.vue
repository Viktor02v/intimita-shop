...
<template>
	...
	<div id="cart" role="dialog" aria-labelledby="cart-title" :aria-hidden="!sidebarStore.isCartOpen" :class="[
		...
		<div class="w-full h-[100vh] flex flex-col justify-center items-center">
			<!-- Loading State -->
			<div v-if="isPendingCartProducts" class="uppercase text-center text-base font-light text-black">
				Loading...
			</div>
			...
			<!-- Empty Cart -->
			<div v-else class="uppercase text-center text-base space-x-2 font-light md:text-black text-white">
				Your cart is empty
			</div>
		</div>
	</div>
