...
<template>
	<div id="cart" role="dialog" aria-labelledby="cart-title" :aria-hidden="!sidebarStore.isCartOpen"
		...
		<!-- Cart Items -->
		<div v-else-if="cartProducts && cartProducts?.length > 0 && user" class="w-full flex flex-col space-y-[50px]">
		...
		<!-- Empty Cart -->
		<div @click="goTo" v-else class="uppercase text-center text-base space-x-2  font-light">
			your cart is empty
		</div>
	</div>
</template>
